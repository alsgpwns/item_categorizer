...
def classify_category(df, text_column, category_type, pipeline_info, max_len=128):
    ...
    log_info(f"{category_type} classification started: rows={len(df)}, device={device}")
    ...
    log_info(f"{category_type} classification completed")
...
def classify_small_fine_category(df, text_column, pipeline_info='./model', moe_type='hard'):
    ...
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    log_info(f"small/fine classification started: rows={len(df)}, device={device}")
    ...
    log_info("small/fine classification completed")
